...
const Member = () => {
  ...
  const { user, userName } = useOutletContext<{ user: any, userName: string }>();
  const [loading, setLoading] = useState(true);
  const [submitting, setSubmitting] = useState(false);
  const [memberData, setMemberData] = useState<any>(null);
  
  // State Form disesuaikan dengan 19 Kolom Database Anda
  ...
  const checkMembership = async () => {
    if (!user) return;
    setLoading(true);
    // FIX: Menggunakan 'email' sebagai kunci karena 'user_id' tidak ada di schema
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Simpan Data Sesuai Schema 19 Kolom
  ...
  if (loading) return <div className="p-20 text-center"><Loader2 className="animate-spin mx-auto text-red-700" size={40}/> <p className="mt-4 font-bold text-gray-500 uppercase text-[10px] tracking-widest">Validasi Data Keanggotaan...</p></div>;
    ...
    return (
      <div className="max-w-3xl mx-auto space-y-6 animate-in zoom-in duration-500">
        <div className="text-center">
          ...
          <p className="text-slate-400 text-[10px] font-bold uppercase tracking-widest">Data Anda telah resmi terdaftar di database Ranting</p>
        </div>
          ...
          <div className="relative z-10 flex flex-col h-full justify-between">
            ...
            <div className="space-y-4 py-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                  ...
                  <div><p className="text-[8px] text-slate-500 uppercase font-black mb-1">NIK</p><p className="font-mono text-xs font-bold">{memberData.nik || '-'}</p></div>
              </div>
              <div>
            ...
            <div className="pt-4 border-t border-white/10 flex justify-between items-end">
                <p className="text-[7px] text-slate-500 italic uppercase">Valid ID: {memberData.id?.slice(0,8)}</p>
        ...
        <div className="flex justify-center"><button onClick={() => window.print()} className="flex items-center gap-2 bg-white border-2 border-slate-200 hover:border-slate-800 px-8 py-3 rounded-2xl text-xs font-black uppercase transition-all shadow-sm"><Printer size={16}/> Cetak Kartu Digital</button></div>
      </div>
    );
  }
      ...
      <div className="bg-white rounded-[40px] shadow-2xl border border-gray-100 overflow-hidden">
        ...
        <form onSubmit={handleRegister} className="p-10 space-y-6">
          <div className="bg-amber-50 p-4 rounded-2xl text-[10px] text-amber-800 flex gap-4 border border-amber-100 font-bold leading-relaxed uppercase">
            ...
            <span>Pastikan NIK 16 digit sesuai KTP untuk mempermudah verifikasi administrasi keanggotaan.</span>
          </div>
          
          <div className="space-y-4">
             <div>
                <label className="text-[10px] font-black uppercase text-gray-400 mb-1 block ml-2">Nama Lengkap & Gelar</label>
                <input required className="w-full p-4 bg-gray-50 border-2 border-gray-100 rounded-[20px] text-xs font-bold uppercase focus:border-red-600 outline-none transition-all" value={formData.full_name} onChange={e => setFormData({...formData, full_name: e.target.value})} />
             </div>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="text-[10px] font-black uppercase text-red-600 mb-1 block ml-2 tracking-widest">NIK (16 Digit)</label>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input className="w-full p-4 bg-gray-50 border-2 border-gray-100 rounded-[20px] text-xs font-bold focus:border-red-600 outline-none" placeholder="-" value={formData.nip} onChange={e => setFormData({...formData, nip: e.target.value})} />
                </div>
                <div>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="text-[10px] font-black uppercase text-gray-400 mb-1 block ml-2">Status Kepegawaian</label>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="text-[10px] font-black uppercase text-gray-400 mb-1 block ml-2">Email Terdaftar</label>
