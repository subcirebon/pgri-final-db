...
interface LoginProps {
  // SEKARANG MENGIRIM DUA DATA: ROLE DAN NAMA
...
const Login = ({ onLogin }: LoginProps) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (password === 'pgri123') {
      if (username === 'super') { 
        onLogin('super_admin', 'SUPER ADMIN'); // Kirim Nama Super Admin
        return; 
      }
      if (username === 'admin') { 
        onLogin('admin', 'ADMIN RANTING'); // Kirim Nama Admin Ranting
        return; 
      }
      if (username === 'user') { 
        onLogin('user', 'ANGGOTA PGRI'); // Kirim Nama Contoh
        return; 
      }
    }
    ...
    if (data) {
      if (data.account_status === 'Active') {
        // MENGAMBIL NAMA ASLI DARI DATABASE
        onLogin('user', data.name); 
      } else {
        ...
        {!isRegistering ? (
          <div className="p-8 space-y-5">
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <button type="submit" disabled={isLoading} className="w-full bg-red-700 hover:bg-red-800 text-white font-bold py-3 rounded-xl shadow-lg flex justify-center items-center gap-2 transition-all">
          ...
          <div className="p-6 space-y-4 animate-in slide-in-from-right">
            ...
            <form onSubmit={handleRegister} className="space-y-3">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                 <input required className="w-full p-2 border rounded-lg text-sm bg-gray-50 uppercase font-semibold" placeholder="Nama Lengkap & Gelar" value={regForm.name} onChange={e => setRegForm({...regForm, name: e.target.value})} />
              ...
              <div className="grid grid-cols-3 gap-3">
                 ...
                 <select className="w-full p-2 border rounded-lg text-sm bg-gray-50 font-bold" value={regForm.gender} onChange={e => setRegForm({...regForm, gender: e.target.value})}><option value="Laki-laki">Laki-laki</option><option value="Perempuan">Perempuan</option></select>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                 ...
                 <input required className="w-full p-2 border rounded-lg text-sm bg-gray-50 uppercase font-semibold" placeholder="Unit Kerja" value={regForm.school} onChange={e => setRegForm({...regForm, school: e.target.value})} />
              </div>

              <div className="grid grid-cols-2 gap-3">
                <select className="w-full p-2 border rounded-lg text-sm bg-gray-50 font-bold" value={regForm.status} onChange={e => setRegForm({...regForm, status: e.target.value as any})}><option value="PNS">PNS</option><option value="PPPK">PPPK</option><option value="Honorer">Honorer</option></select>
                <select className="w-full p-2 border rounded-lg text-sm bg-gray-50 font-bold" value={regForm.teacher_type} onChange={e => setRegForm({...regForm, teacher_type: e.target.value})}><option value="Guru Kelas">Guru Kelas</option><option value="Guru PAI">Guru PAI</option><option value="Guru PJOK">Guru PJOK</option><option value="Kepala Sekolah">Kepala Sekolah</option></select>
              </div>
              ...
              <div className="bg-red-50 p-4 rounded-xl border border-red-100 mt-2">
                <p className="text-[10px] font-black text-red-800 uppercase mb-2 flex items-center gap-1"><ShieldCheck size={14}/> Buat Akses Login Anda</p>
                <div className="grid grid-cols-2 gap-3">
                  <input required className="w-full p-2 border border-red-200 rounded-lg text-sm font-bold" placeholder="Username" value={regForm.reg_username} onChange={e => setRegForm({...regForm, reg_username: e.target.value})} />
                  <input required type="password" className="w-full p-2 border border-red-200 rounded-lg text-sm font-bold" placeholder="Password" value={regForm.reg_password} onChange={e => setRegForm({...regForm, reg_password: e.target.value})} />
                </div>
              </div>
