    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-xs text-gray-500 italic">Ditemukan: {filteredMembers.length} Orang</p>
        </div>
        {isAdmin && (
          <button onClick={() => { setIsEditing(false); setFormData({npa: '', name: '', nip: '', birth_place: '', birth_date: '', gender: 'Laki-laki', school: '', status: 'PNS', teacher_type: 'Guru Kelas', phone: '', email: ''}); setShowModal(true); }} className="bg-red-800 text-white px-4 py-2 rounded-lg font-bold shadow-lg flex items-center gap-2 hover:bg-red-900">
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border flex flex-col md:flex-row gap-3 items-center">
        <div className="relative flex-1 w-full">
          ...
          <input type="text" placeholder="Cari Nama/NIP/NPA..." className="w-full pl-10 pr-4 py-2 border rounded-lg outline-none focus:ring-2 focus:ring-red-500 text-sm" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        </div>
        <div className="flex gap-2 w-full md:w-auto">
          ...
          <select className="p-2 border rounded-lg text-xs font-bold bg-gray-50 outline-none" value={filterType} onChange={(e) => setFilterType(e.target.value)}>
            <option value="Semua">Jenis: Semua</option>
            <option value="Guru Kelas">Guru Kelas</option><option value="Guru PAI">Guru PAI</option><option value="Guru PJOK">Guru PJOK</option><option value="Guru B. Inggris">Guru B. Inggris</option><option value="Kepala Sekolah">Kepala Sekolah</option>
          </select>
                ...
                {filteredMembers.map((m) => (
                  <tr key={m.id} className="hover:bg-gray-50 transition-colors">
                    <td className="p-4">
                      ...
                      {/* --- BAGIAN YANG DIPERBAIKI: NIP SEKARANG MUNCUL --- */}
                      <div className="text-[10px] text-gray-500 font-mono">
                        {m.nip ? `NIP: ${m.nip}` : 'NIP: -'} | {m.npa ? `NPA: ${m.npa}` : 'NPA: -'}
                      </div>
                    </td>
                    <td className="p-4 text-[11px]">
                      <div>{m.birth_place || '-'}, {m.birth_date || '-'}</div>
                      <div className="text-gray-400 font-bold">{m.gender}</div>
                    </td>
                    <td className="p-4">
                    ...
                    <td className="p-4 text-center">
                      <span className={`px-2 py-1 rounded text-[10px] font-bold border ${m.status === 'PNS' ? 'bg-green-50 text-green-700 border-green-200' : m.status === 'PPPK' ? 'bg-blue-50 text-blue-700 border-blue-200' : 'bg-orange-50 text-orange-700 border-orange-200'}`}>{m.status}</span>
                    </td>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-lg p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleDirectAddSubmit} className="space-y-3 text-sm">
               ...
               <div className="grid grid-cols-2 gap-3">
                  <div><label className="font-bold text-xs text-gray-500 uppercase">WA</label><input className="w-full p-2 border rounded" value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} /></div>
               ...
               <div className="flex gap-2 pt-4">
                 ...
                 <button type="submit" disabled={loading} className="flex-1 py-3 bg-red-800 text-white rounded-xl font-bold shadow-lg hover:bg-red-900">{loading ? 'Proses...' : 'Simpan Anggota'}</button>
               </div>
            </form>
          </div>
        </div>
