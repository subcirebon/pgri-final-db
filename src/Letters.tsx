...
const URL_TTD_DEFAULT = "https://vuzwlgwzhiuosgeohhjl.supabase.co/storage/v1/object/public/letters-archive/ttd-surat-removebg.png";
...
const Letters = () => {
  ...
  const handleDeleteLetter = async (table: string, id: string) => {
    if(!window.confirm("Hapus data arsip ini?")) return;
    try {
  ...
  const handleArchiveFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const fileName = `outgoing/${Date.now()}_scan.${fileExt}`;
      ...
      alert("Scan Berhasil Diunggah!"); fetchData(); 
    ...
    try {
      ...
      const docDef: any = {
        ...
        content: [
          ...
          { canvas: [{ type: 'line', x1: 0, y1: 0, x2: 470, y2: 0, lineWidth: 2.5 }], margin: [0, 2, 0, 0] },
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center bg-white p-6 rounded-[32px] border shadow-sm gap-4">
        ...
        <div className="flex bg-gray-100 p-1 rounded-2xl shadow-inner">
          ...
          <button onClick={() => setActiveTab('in')} className={`px-5 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === 'in' ? 'bg-white shadow-sm text-blue-700' : 'text-gray-400'}`}>MASUK</button>
          <button onClick={() => setActiveTab('out')} className={`px-5 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === 'out' ? 'bg-white shadow-sm text-green-700' : 'text-gray-400'}`}>KELUAR</button>
        </div>
      </div>
      ...
      {activeTab === 'create' && isAdmin && !isPreviewing && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in slide-in-from-bottom-4 duration-500">
           <div className="lg:col-span-2 bg-white p-8 rounded-[32px] border shadow-sm space-y-5">
            ...
            {selectedType.formType !== 'formal' && <textarea rows={2} className="w-full p-3 border rounded-xl font-bold outline-none focus:border-red-600" placeholder="Tujuan (Yth. ...)" value={formData.tujuan} onChange={e => setFormData({...formData, tujuan: e.target.value})} />}
      ...
      {activeTab === 'out' && (
        <div className="bg-white rounded-[32px] border p-6 shadow-sm">
           ...
           <div className="space-y-3">
             {lettersOut.map(l => (
               <div key={l.id} className="p-4 bg-gray-50 border rounded-2xl flex justify-between items-center hover:bg-white group transition-all">
                 <div><p className="font-bold text-sm uppercase text-gray-800">{l.subject}</p><p className="text-[10px] text-gray-400 font-bold uppercase">No: {l.letter_number}</p></div>
                 <div className="flex items-center gap-3">
                    {l.file_url ? (
                      <div className="flex items-center gap-2">
                        ...
                        {isAdmin && <button onClick={() => triggerUploadArchive(l.id)} className="p-1.5 text-gray-400 hover:text-blue-600">{uploadingId === l.id ? <Loader2 className="animate-spin" size={14}/> : <Edit3 size={16}/>}</button>}
                      </div>
                    ) : isAdmin && (
                      <button onClick={() => triggerUploadArchive(l.id)} className="text-orange-600 text-[10px] font-bold border border-orange-100 px-3 py-1.5 rounded-lg bg-orange-50 hover:bg-orange-500 hover:text-white flex items-center gap-1">
      ...
      {isPreviewing && (
        <div className="fixed inset-0 bg-gray-900 z-50 overflow-y-auto font-sans">
           <div className="bg-slate-800 p-4 sticky top-0 flex justify-between items-center text-white px-8 shadow-xl">
              <button onClick={() => setIsPreviewing(false)} className="px-5 py-2 font-bold text-sm bg-slate-700 rounded-xl uppercase transition-all">← Edit Kembali</button>
              <button onClick={handleDirectPrint} disabled={uploading} className="px-7 py-2 bg-blue-600 rounded-xl font-bold uppercase shadow-xl flex gap-2 hover:bg-blue-700 transition-all">
                 ...
                 <div className="text-[12pt] space-y-6 leading-relaxed">
                    {selectedType.formType === 'formal' ? ( 
                        <div className="text-center space-y-1 mb-8"> 
                            <h3 className="text-[14pt] font-bold underline leading-none uppercase">{selectedType.label}</h3> 
