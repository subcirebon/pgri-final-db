...
const Letters = () => {
  ...
  const generatePDF = (l: Letter, action: 'preview' | 'download') => {
    ...
    // 3. LOGIKA ISI SURAT (BERBEDA TIAP TIPE)
    if (l.type === 'UNDANGAN') {
      ...
      // FORMAT SURAT LAINNYA (PARAGRAF BEBAS)
      const splitBody = doc.splitTextToSize(l.body || '', 175);
      doc.text(splitBody, 20, marginAtas + 70);
      const nextY = marginAtas + 75 + (splitBody.length * 6);
      doc.text('Demikian surat ini kami sampaikan untuk dipergunakan sebagaimana mestinya.', 20, nextY);
    }
      ...
      <div className="flex justify-between items-end bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        ...
        {isAdmin && (
          <div className="flex gap-2">
            <button onClick={() => { setFormData({date: new Date().toISOString().split('T')[0], ref_number: '', subject: '', type: 'UNDANGAN', sender_receiver: '', attachment: '-', event_date: '', venue: '', agenda: '', body: ''}); setIsEditing(false); setShowModal(true); }} className="bg-blue-600 text-white px-5 py-2.5 rounded-xl font-bold text-xs flex items-center gap-2 shadow-lg hover:bg-blue-700"><Mail size={16} /> Undangan</button>
            <button onClick={() => { setFormData({date: new Date().toISOString().split('T')[0], ref_number: '', subject: '', type: 'LAINNYA', sender_receiver: '', attachment: '-', event_date: '', venue: '', agenda: '', body: ''}); setIsEditing(false); setShowModal(true); }} className="bg-teal-600 text-white px-5 py-2.5 rounded-xl font-bold text-xs flex items-center gap-2 shadow-lg hover:bg-teal-700"><FilePlus size={16} /> Surat Lainnya</button>
          </div>
        )}
      </div>
          ...
          <tbody className="divide-y">
            {letters.map((l) => (
              <tr key={l.id} className="hover:bg-gray-50 transition-colors group">
                <td className="p-4"><div className="text-[10px] text-gray-400">{formatTanggalIndo(l.date)}</div><div className="font-bold text-red-800">{l.ref_number}</div></td>
                <td className="p-4 font-bold text-gray-700">{l.subject}</td>
                <td className="p-4"><span className={`px-2 py-0.5 rounded text-[9px] font-black ${l.type === 'UNDANGAN' ? 'bg-blue-100 text-blue-700' : 'bg-teal-100 text-teal-700'}`}>{l.type}</span></td>
      ...
      {showPreview && pdfUrl && (
        <div className="fixed inset-0 bg-black/80 z-[60] flex flex-col p-4 backdrop-blur-md">
          <div className="flex justify-between items-center bg-white p-4 rounded-t-2xl border-b">
            <div className="flex items-center gap-3"><button onClick={() => setShowPreview(false)} className="p-2 hover:bg-gray-100 rounded-full text-gray-600"><ArrowLeft size={20}/></button><h3 className="font-bold text-gray-800 italic">Preview Cetak TNR</h3></div>
            <div className="flex gap-2"><button onClick={() => setShowPreview(false)} className="px-5 py-2 border rounded-xl font-bold text-sm text-gray-600">Kembali</button><button onClick={() => generatePDF(currentLetter!, 'download')} className="px-5 py-2 bg-blue-600 text-white rounded-xl font-bold text-sm flex items-center gap-2 shadow-lg"><Download size={16}/> Download PDF</button></div>
          </div>
          <div className="flex-1 bg-gray-600/50 rounded-b-2xl overflow-hidden flex justify-center"><iframe src={pdfUrl} className="w-full max-w-4xl h-full shadow-2xl bg-white" title="PDF Preview"></iframe></div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-3xl w-full max-w-xl p-8 shadow-2xl max-h-[95vh] overflow-y-auto">
            <div className="flex justify-between items-start border-b pb-4 mb-6">
              ...
              <div className="text-right"><span className="bg-red-50 text-red-700 px-2 py-1 rounded text-[10px] font-bold border border-red-100">No. Terakhir: {lastRef || '-'}</span></div>
            </div>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div><label className="text-[10px] font-bold text-gray-400 uppercase">Tanggal</label><input type="date" className="w-full p-3 bg-gray-50 border rounded-xl" value={formData.date} onChange={e => setFormData({...formData, date: e.target.value})} /></div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div><label className="text-[10px] font-bold text-gray-400 uppercase">Lampiran</label><input className="w-full p-3 bg-gray-50 border rounded-xl" value={formData.attachment} onChange={e => setFormData({...formData, attachment: e.target.value})} /></div>
              ...
              {formData.type === 'UNDANGAN' ? (
                <div className="bg-blue-50/50 p-6 rounded-2xl space-y-4 border border-blue-100">
                  ...
                  <input required className="w-full p-2.5 bg-white border border-blue-100 rounded-lg" placeholder="Hari/Tanggal" value={formData.event_date} onChange={e => setFormData({...formData, event_date: e.target.value})} />
                  <input required className="w-full p-2.5 bg-white border border-blue-100 rounded-lg" placeholder="Tempat" value={formData.venue} onChange={e => setFormData({...formData, venue: e.target.value})} />
                  <input required className="w-full p-2.5 bg-white border border-blue-100 rounded-lg" placeholder="Acara" value={formData.agenda} onChange={e => setFormData({...formData, agenda: e.target.value})} />
                </div>
              ) : (
                <div className="bg-teal-50/50 p-6 rounded-2xl border border-teal-100">
                  <label className="text-[10px] font-bold text-teal-700 uppercase underline mb-2 block">Isi Surat (Paragraf Tengah)</label>
                  <textarea required className="w-full p-3 bg-white border border-teal-100 rounded-xl h-40 outline-none" placeholder="Ketik isi surat di sini..." value={formData.body} onChange={e => setFormData({...formData, body: e.target.value})}></textarea>
                </div>
              )}
