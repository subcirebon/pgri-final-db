...
const Info = () => {
  // Ambil userRole dari Layout untuk cek admin
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isEditing && editId) {
      const { error } = await supabase.from('news').update(formData).eq('id', editId);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        ...
        {isAdmin && (
          <button 
            onClick={() => { setIsEditing(false); setFormData({date: new Date().toISOString().split('T')[0], title: '', content: '', category: 'Berita', author: 'Admin Ranting', image_url: ''}); setShowModal(true); }} 
              ...
              <div className="p-4 px-5 border-t bg-gray-50 flex justify-between items-center mt-auto">
                ...
                {isAdmin && (
                  <div className="flex gap-1">
                    ...
                    <button onClick={() => handleDelete(n.id)} className="p-2 text-red-600 hover:bg-red-100 rounded-lg"><Trash2 size={14} /></button>
                  </div>
                )}
              </div>
            </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl w-full max-w-2xl p-8 shadow-2xl overflow-y-auto max-h-[90vh]">
            <h3 className="font-black text-xl mb-6 uppercase italic border-b pb-4 text-gray-800">{isEditing ? 'Edit Berita' : 'Tulis Berita Baru'}</h3>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <label className="cursor-pointer bg-blue-50 text-blue-700 px-4 py-2 rounded-xl font-bold text-xs flex items-center gap-2 hover:bg-blue-100 transition-all">
                    {uploading ? <Loader2 className="animate-spin" size={14}/> : <Upload size={14}/>} {formData.image_url ? 'Ganti Foto' : 'Upload Foto'}
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <select className="p-3 bg-gray-50 border rounded-xl font-bold outline-none" value={formData.category} onChange={e => setFormData({...formData, category: e.target.value as any})}><option value="Berita">BERITA</option><option value="Pengumuman">PENGUMUMAN</option><option value="Agenda">AGENDA</option></select>
              </div>
              ...
              <textarea required className="w-full p-3 bg-gray-50 border rounded-xl h-40 outline-none focus:ring-2 focus:ring-red-500 text-sm" placeholder="Isi berita..." value={formData.content} onChange={e => setFormData({...formData, content: e.target.value})}></textarea>
              ...
              <div className="flex gap-3 pt-4 border-t mt-4">
                ...
                <button type="submit" disabled={loading || uploading} className="flex-1 py-4 bg-red-800 text-white rounded-2xl font-black shadow-xl hover:bg-red-900 uppercase text-[10px] tracking-widest transition-all">Simpan</button>
              </div>
            </form>
          </div>
        </div>
